...
为了便于充分理解以下说明和示例返回，建议在 `$GOPATH/github.com/kumasuke120/mockuma` 目录下执行以下命令，以启动示例配置的 Mockuma：
```
...
| method | （选填，默认 Any）Mock 接口绑定的请求方式，支持所有 [Http/1.1 的请求方式](https://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html) 以及 Any；<br>其中前者为单独映射， Any 则映射所有请求方式 | `"GET"` |
