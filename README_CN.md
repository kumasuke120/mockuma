...
#### 特殊指令支持
- `when` 和 `body` 中支持 `@file` 文件指令，该指令指定一个文件路径（相对路径是相对 `mapfile` 所在目录），并读取其内容作为该参数的值。
推荐在**返回体较大**时使用该指令，可以使得配置文件更加简洁。
- `when` 中支持 `@regexp` 正则表达式指令，该指令指定一个正则表达式，用于匹配 when 中的参数。
如果在诸如 `params` 或 `headers` 等存在一名称多值的参数中使用，只能生效一个正则表达式，且只要请求中传入的参数有一个匹配即匹配。
- `when` 中支持 `@json` Json 匹配指令，该指令指定一个 Json 结构，匹配时将会将对应参数解析为 Json 格式，并进行匹配。
只要请求中的参数满足指定的 Json 结构（而不是相等）即可匹配。
如果在诸如 `params` 或 `headers` 等存在一名称多值的参数中使用，只能生效一个 Json 匹配，且只要请求中传入的参数有一个匹配即匹配。
